...
async function printAddressNoUp(contractName, publicAddress) {
  console.log(`${contractName} Public Address: ${publicAddress}`);  
}
...
async function deploySCNoUp(contractName, args = []) {
  ...
  console.log("--------------------- deploySCNoUp -----------------");
  var SmartContract = await gcf(contractName);
  var smartContract = await SmartContract.deploy([...args]);

  // true cuando se usa '--network matic' en el script de deployment
  if (process.env.HARDHAT_NETWORK) {
    //console.log("process.env.HARDHAT_NETWORK:", process.env.HARDHAT_NETWORK);
